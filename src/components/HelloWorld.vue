...
<li class="nav-item">
<a href="https://developers.ripple.com/build-run-rippled-ubuntu.html" target="_blank" class="btn btn-outline-primary btn-xs inactive" role="button" aria-pressed="true"> BUILD RIPPLED </a>
</li>
</ul>
</span>
      </div>
  ...
  <div class="jumbotron" text-variant="secondary" border-variant="dark" align="center">
    ...
    <div class="scroller"  align="left">XRP
      ...
      <span class="cursor" v-html="cursor"></span>
        ...
        <div class="col-4">
          <div align="right" style="color: black; font-size: 13px;">
            Built with <a class="ripple" href="https://developers.ripple.com/build-run-rippled-ubuntu.html" target="_blank" style="text-decoration:none;">RIPPLED</a> version <span style="color: #400080;" v-if="serverState !== null"> {{ serverState.build_version }}</span></div>
        </div>
      </div>
    </div>
  </div>
          ...
          <div id="collapseOne" class="collapse" aria-labelledby="headingOne" data-parent="#accordian">
            <div class="card-body">
              <div class="row">
                ...
                <div class="serverrt col-12 col-sm-9"><b v-if="serverState !== null"> <span class="text-success">{{ serverState.server_state }}</span></b></div>
                ...
                <div class="serverrt col-12 col-sm-9"><span v-if="serverState !== null"> {{ serverState.build_version }}</span></div>
                ...
                <div class="serverrt col-12 col-sm-9"><span v-if="serverState !== null">{{ Math.round(serverState.uptime/86400 * 100)/ 100}} days</span></div>
                ...
                <div class="serverrt col-12 col-sm-9"><b v-if="serverState !== null"> {{ serverState.validated_ledger.seq }}</b></div>
                ...
                <div class="serverrt col-12 col-sm-9"><span v-if="serverState !== null"> {{ serverState.complete_ledgers }}</span></div>


                <div class="serverst d-none d-sm-block col-sm-3 text-right">Last Ledger #</div>
                <div class="serverst d-block d-sm-none col-12 text-left text-muted">Last Ledger #</div>
                <div class="serverrt col-12 col-sm-9"><span v-if="serverState !== null"> {{ serverState.validated_ledger.hash }}</span></div>
                ...
                <div class="serverrt col-12 col-sm-9"><span v-if="serverState !== null"> {{ serverState.pubkey_node }}</span></div>
              </div>
            </div>
          </div>
               ...
               <div v-else>
                 <div class="table-responsive" style="margin-left: -11px; margin-top: -21px;">
                   ...
                   <tbody>
                     ...
                     <td  class="Tab4"><a v-bind:href="'https://xrpcharts.ripple.com/#/validators/' + q.validation_public_key" target="_blank" style="text-decoration:none;" > {{ q.validation_public_key }}</a></td>
                     <td  class="Tab4" > <p style="text-align: center; color: darkgreen; margin-bottom: 0; font-size: 20px;" v-if="q.domain_state"> &#10004; </p></td>          
                     <td  class="Tab4"><p style="margin-bottom: 0;" v-if="q.domain_state"><p style="margin-bottom: 0; color: darkblue; font-style: italic;" v-else> not yet verified</p> <a v-bind:href="q.domain" target="_blank" style="text-decoration:none;">{{ q.domain }}</a></td>
                     </tr>
                   </tbody>
                 </table>
                 </div>
              ...
              <div class="card-header" id="headingFive" align="left">
                <h5 class="mb-0">
                  <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseFive"
                   ...
                   XRP Transactions - Previous 24hrs - by Exchange 
                  </button>
                        ...
                        <table class="table table-striped">
                          ...
                          <tbody>
                            <tr style="color: darkblue;" scope="row">
                              <td />
                              <td>{{ new Date(Date.parse(xrpData.data.date)).toLocaleDateString('en-NZ',{ weekday: 'long', month:
                                'long', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit', second:
                                '2-digit', timeZoneName: 'short'}).replace(',',' ') }}</td>
                              <td />
                              <td>Last 24 hours</td>
                              <td />
                              <td>{{ xrpData.data.total }} xrp</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
...
export default {
  ...
  data() {
    return {
      ...
      'is scalable beyond 50,000 transactions per second',
      'is stable - all 30 million ledgers closed without issue',
